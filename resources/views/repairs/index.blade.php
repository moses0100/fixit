...
<div class="panel">
<form method="GET" class="filters row g-3 m-0" action="{{ route($admin ? 'admin.repairs.index' : 'repairs.index') }}">
    <div class="col-lg-4"><label class="form-label" for="q">ค้นหารายการ</label><input class="form-control" id="q" name="q" value="{{ request('q') }}" maxlength="150" placeholder="เลขแจ้งซ่อม / อุปกรณ์ / Serial / ชื่อผู้แจ้ง"></div>
...
<div class="panel-head"><h2>รายการทั้งหมด <span class="text-muted fw-normal small">/ {{ $repairs->total() }} รายการ</span></h2></div>
@include('repairs.table')
@if($repairs->hasPages())<div class="p-4">{{ $repairs->links() }}</div>@endif
</div>
@endsection
