...
<label class="form-label mt-3" for="contact_phone">เบอร์ติดต่อ <span class="req">*</span></label><input class="form-control @error('contact_phone') is-invalid @enderror" id="contact_phone" name="contact_phone" type="tel" inputmode="tel" autocomplete="tel" required maxlength="30" pattern="[0-9+() .-]{8,30}" value="{{ old('contact_phone', $repair->contact_phone) }}" placeholder="08x xxx xxxx" aria-describedby="phone-help">@error('contact_phone')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
<div class="form-text" id="phone-help">ใช้สำหรับติดต่อเกี่ยวกับงานซ่อมนี้ ตัวเลข 8-30 ตัว</div>
</div></div>
